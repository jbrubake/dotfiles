...
function! s:getProperty(key, ...)
    ...
    " If a 2nd argument has been passed in, use it as the value,
else 0
...
function! s:history(goNewer)
    ...
    " Apply the cmd repeatedly until we hit a non-empty list, or
    " first/last list is reached
    while 1
        if (a:goNewer && s:isLast()) || (!a:goNewer && s:isFirst()) | break | endif
        " Run the command. Use :silent to suppress message-history
        " output.  Note that the :try wrapper is no longer necessary
        silent execute l:cmd
        if s:length() | break | endif
    endwhile

    " Set the height of the quickfix window to the
    " size of the list, max-height 10
...
function!  qfhistory#older()
    call s:history(0)
endfunction

function!  qfhistory#newer()
    call s:history(1)
endfunction
